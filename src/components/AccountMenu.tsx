    ...
    <>
      <div className="relative">
        ...
        {showMenu && (
          <>
            ...
            />
            <div className="absolute right-0 top-12 z-50 w-72 bg-background rounded-2xl shadow-sm border border-gray-100 p-2 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-3 py-2 border-b border-gray-100 mb-2">
                <p className="text-sm font-semibold truncate">{user?.email}</p>
                <p className="text-sm text-gray-600">Synced across devices</p>
              </div>

              {profileLink && (
                <button
                  onClick={handleCopyLink}
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-muted transition-colors text-left mb-1"
                >
                  <LinkIcon className="w-4 h-4 text-muted-foreground" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold">Share Profile</p>
                    <p className="text-sm text-gray-600 truncate">{profileLink}</p>
                  </div>
                  {copied ? (
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-background rounded-2xl shadow-sm border border-gray-100 p-6 animate-in fade-in zoom-in-95 duration-200">
